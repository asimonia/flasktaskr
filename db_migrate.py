# migrations script to update the existing schema
...
with sqlite3.connect(DATABASE_PATH) as connection:
	...
	# recreate a new tasks table with schema
	db.create_all()
	...
	data = [(row[0], row[1], row[3], datetime.now(), 1) for row in c.fetchall()]

	# insert data to tasks table
	c.executemany("""INSERT INTO tasks (name, due_date, priority, status, posted_date, user_id) VALUES
						(?, ?, ?, ?, ?, ?)""", data)
